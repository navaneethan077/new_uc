...
export default function AnnouncementsPage() {
  ...
  const categories = useMemo(() => {
    return Array.from(new Set(announcements.map((item: Announcement) => item.category)))
  }, [])
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    filtered.sort((a: Announcement, b: Announcement) => b.date.localeCompare(a.date))
    return filtered
  ...
  const handleNoticeClick = (noticeId: string): void => {
    router.push(`/events/notices?id=${noticeId}`)
  }

  const viewAllNotices = (): void => {
    router.push("/events/notices")
  }

  const lastFiveNotices: Notice[] = [...notices]
    .sort((a: Notice, b: Notice) => new Date(b.dateIssued).getTime() - new Date(a.dateIssued).getTime())
    .slice(0, 5)
              ...
              <div className="lg:col-span-8 space-y-6">
                {visibleItems.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    {visibleItems.map((announcement: Announcement) => {
              ...
              <div className="lg:col-span-4">
                <div className="bg-white rounded-xl md:rounded-2xl shadow-lg p-4 md:p-6 sticky top-28 md:top-32">
                  ...
                  <div className="space-y-3 md:space-y-4">
                    {lastFiveNotices.map((item: Notice) => (
                      <div
                        key={item.id}
